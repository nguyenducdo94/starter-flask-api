...
groups = config.get('Settings', 'groups').split(', ')
forward_group = config.get('Settings', 'forward_group').split(', ')
my_username = config.get('Settings', 'my_username')

# Get values from the 'API' section
api_id = config.get('API', 'api_id')
api_hash = config.get('API', 'api_hash')
phone_number = config.get('API', 'phone_number')


# Định nghĩa một hàm xử lý tin nhắn mới
async def handle_new_message(event):
    ...
    if isinstance(event.message, types.Message):
        ...
        if event.photo:
            await event.download_media(unique_filename)

        for group_username in forward_group:
            if event.photo:
                await client.send_file(group_username, unique_filename, caption=event.message.message)
                print(f'Đã forward tin nhắn tới group {group_username}')
            else:
                await client.send_message(group_username, event.message.message)
                print(f'Đã forward tin nhắn tới group {group_username}')

        if os.path.exists(unique_filename):
            os.remove(unique_filename)

# Tạo một client Telethon
client = TelegramClient(phone_number, api_id, api_hash)

client.start()

target_user = client.get_entity(my_username)
my_id = target_user.id

print(f'The user ID of {my_username} is {my_id}')
